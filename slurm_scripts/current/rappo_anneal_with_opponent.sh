experiment_name=2026_02_21_rappo_anneal_opponent
...
#SBATCH --job-name=anneal_ragnn_opp_s${seed}                                           # Name of the job
...
PYTHONPATH=$(pwd) python training_scripts/train_ppo.py -f configs/rappo_anneal_with_opponent.yaml -wd . -s ${seed} -j 0 --model-type RAGNN --experiment-name ${experiment_name} --opponent
EOF
done
