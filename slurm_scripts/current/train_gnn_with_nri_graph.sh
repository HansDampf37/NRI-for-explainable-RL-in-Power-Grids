experiment_name=2026_20_03_fixed_graph
...
#PYTHONPATH=$(pwd) python training_scripts/train_ppo.py -f configs/gnn_rappo_graph.yaml -wd . -s 0 -j 0 --model-type NRIGNN --experiment-name ${experiment_name} --opponent
#EOF
...
#SBATCH --output=results/experiments/${experiment_name}/out/fixed_graph.%j.log            # Output file
#SBATCH --error=results/experiments/${experiment_name}/out/fixed_graph.%j.log             # Error file
#SBATCH --ntasks=1                                                            # Number of tasks
#SBATCH --cpus-per-task=111                                                   # Number of CPU cores per task
#SBATCH --time=20:00:00                                                       # Max wall time (HH:MM:SS)
#SBATCH --mem=200G                                                            # Memory requirement
