...
#SBATCH --job-name=anneal_ragnn_s${seed}                                           # Name of the job
...
PYTHONPATH=$(pwd) python training_scripts/train_ppo.py -f configs/rappo_anneal.yaml -wd . -s ${seed} -j 0 --model-type RAGNN --experiment-name ${experiment_name}
EOF
done
