...
class Hypothesis1verifier(PosteriorAnalyzer):
    ...
    def generate_plots(self, C_T: ndarray[Any, dtype[Any]], P_T: ndarray[Any, dtype[Any]],
        ...
        if self.compare_to_mean_coupling:
            plt.xlabel(f"{cl} " + r"$\bar{C}_{ij}^{PTDF}$")
        else:
            plt.xlabel(f"{cl} " + r"$C_{ij}^{PTDF}$")
        plt.ylabel("Density")
        if self.compare_to_mean_coupling:
            plt.title(r"Average $\bar{C}_{ij}^{PTDF}$ conditioned on posterior")
        else:
            plt.title(r"$C_{ij}^{PTDF}$ conditioned on posterior")
        plt.legend()
        plt.tight_layout()
        ...
        #self._save_hist(
        #    edge_corr,
        #    title=f"Temporal Pearson: PTDF coupling vs posterior",
...
def main():
    #Hypothesis1verifier().repaint()
    #c_t = np.load("/home/adrian/Dev/NRI-for-explainable-RL-in-Power-Grids/results/hypothesis_electrical_coupling/coupling_over_time.npy")
    #print(c_t.shape)
    #sns.histplot(c_t.var(axis=0))
    #plt.show()
    #sns.histplot(c_t.mean(axis=0))
    #plt.show()
    ...
    #analysis_agent = LatentGraphAnalysisAgent(agent, gym_env, Hypothesis1verifier(compare_to_mean_coupling=True))
    #logger.info("Agent loaded! Starting episodes...\n")
    #analysis_agent.analyze(num_episodes=50)

    Hypothesis1verifier(compare_to_mean_coupling=False).repaint()
    Hypothesis1verifier(compare_to_mean_coupling=True).repaint()
