...
class G2OpDQN(DQN):
    def dump_logs(self) -> None:
        ...
        if len(self.ep_info_buffer) > 0 and len(self.ep_info_buffer[0]) > 0:
            ...
            self.logger.record("rollout/action_entropy_mean", safe_mean([ep_info.get("action_entropy", 0.0) for ep_info in self.ep_info_buffer]))
            self.logger.record("rollout/unique_action_ratio_mean", safe_mean([ep_info.get("unique_action_ratio", 0.0) for ep_info in self.ep_info_buffer]))
            self.logger.record("rollout/unique_actions_mean", safe_mean([ep_info.get("unique_actions", 0) for ep_info in self.ep_info_buffer]))
