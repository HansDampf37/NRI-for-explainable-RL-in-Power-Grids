...
class TestHydra(unittest.TestCase):
    def test_hydra_obs_space(self):
        with hydra.initialize(config_path="../hydra_configs", version_base="1.3"):
            cfg = hydra.compose(config_name="obs_space/graph_obs_space")
    ...
    def test_hydra_obs_space_2(self):
        with hydra.initialize(config_path="../hydra_configs", version_base="1.3"):
            cfg = hydra.compose(config_name="obs_space/box_obs_space")
            env = grid2op.make("l2rpn_case14_sandbox")
            obs_space = instantiate(cfg.obs_space, grid2op_observation_space=env.observation_space)
    ...
    def test_hydra_env_creation(self):
        with hydra.initialize(config_path="../hydra_configs", version_base="1.3"):
            ...
            obs_cfg = hydra.compose(config_name="obs_space/box_obs_space")
            act_cfg = hydra.compose(config_name="act_space/discrete_set_bus")
            env = instantiate(
                env_cfg.env,
                obs_space_creation = lambda e: instantiate(obs_cfg.obs_space, grid2op_observation_space=e.observation_space),
                act_space_creation = lambda e: instantiate(act_cfg.act_space, grid2op_action_space=e.action_space)
            )
